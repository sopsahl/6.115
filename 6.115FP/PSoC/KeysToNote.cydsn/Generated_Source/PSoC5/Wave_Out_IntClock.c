...
/*******************************************************************************
* Function Name: Wave_Out_IntClock_Start
********************************************************************************
*
* Summary:
...
void Wave_Out_IntClock_Start(void) 
{
    ...
    Wave_Out_IntClock_CLKEN |= Wave_Out_IntClock_CLKEN_MASK;
	Wave_Out_IntClock_CLKSTBY |= Wave_Out_IntClock_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: Wave_Out_IntClock_Stop
********************************************************************************
*
* Summary:
...
void Wave_Out_IntClock_Stop(void) 
{
    ...
    Wave_Out_IntClock_CLKEN &= (uint8)(~Wave_Out_IntClock_CLKEN_MASK);
	Wave_Out_IntClock_CLKSTBY &= (uint8)(~Wave_Out_IntClock_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: Wave_Out_IntClock_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((Wave_Out_IntClock_CLKEN & Wave_Out_IntClock_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = Wave_Out_IntClock_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = Wave_Out_IntClock_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* Wave_Out_IntClock__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(Wave_Out_IntClock_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(Wave_Out_IntClock_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: Wave_Out_IntClock_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: Wave_Out_IntClock_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = Wave_Out_IntClock_GetSourceRegister();
    uint16 oldDivider = Wave_Out_IntClock_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = Wave_Out_IntClock_CLKEN & Wave_Out_IntClock_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(Wave_Out_IntClock_DIV_PTR, clkDivider);
                Wave_Out_IntClock_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                Wave_Out_IntClock_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(Wave_Out_IntClock_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((Wave_Out_IntClock_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
            ...
            else
            {
                ...
                CY_SET_REG16(Wave_Out_IntClock_DIV_PTR, clkDivider);
				Wave_Out_IntClock_CLKEN |= enabled;
            }
        }
    }
}
...
/*******************************************************************************
* Function Name: Wave_Out_IntClock_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(Wave_Out_IntClock_DIV_PTR);
}


/*******************************************************************************
* Function Name: Wave_Out_IntClock_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    Wave_Out_IntClock_MOD_SRC |= modeBitMask & (uint8)Wave_Out_IntClock_MODE_MASK;
}


/*******************************************************************************
* Function Name: Wave_Out_IntClock_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    Wave_Out_IntClock_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(Wave_Out_IntClock_MODE_MASK));
}


/*******************************************************************************
* Function Name: Wave_Out_IntClock_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return Wave_Out_IntClock_MOD_SRC & (uint8)(Wave_Out_IntClock_MODE_MASK);
}


/*******************************************************************************
* Function Name: Wave_Out_IntClock_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = Wave_Out_IntClock_GetDividerRegister();
    uint8 oldSrc = Wave_Out_IntClock_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        Wave_Out_IntClock_MOD_SRC |= CYCLK_SSS;
        Wave_Out_IntClock_MOD_SRC =
            (Wave_Out_IntClock_MOD_SRC & (uint8)(~Wave_Out_IntClock_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: Wave_Out_IntClock_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return Wave_Out_IntClock_MOD_SRC & Wave_Out_IntClock_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: Wave_Out_IntClock_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    Wave_Out_IntClock_PHASE = clkPhase & Wave_Out_IntClock_PHASE_MASK;
}


/*******************************************************************************
* Function Name: Wave_Out_IntClock_GetPhase
********************************************************************************
*
* Summary:
...
{
    return Wave_Out_IntClock_PHASE & Wave_Out_IntClock_PHASE_MASK;
}
