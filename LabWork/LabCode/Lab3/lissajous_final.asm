; this program makes the lissajous curves with rotation
...
start: 
	lcall init
	lcall digitWait ; infinite loop
	
init:
	mov P1, #0ffh ; imitializing the p1
	...
	; initializing the offsets (starting us with the tie fighter)
	mov R4, #00h ; x low
	mov R5, #01h ; x high
	mov R6, #00h ; y low
	mov R7, #03h ; y high
...
circle:
	...
	ljmp digitWait
curve:	
	cjne a, #02h, rotation_addition
	mov R5, #01h ; x high
	mov R7, #04h ; y high
	mov R3, #00h ; no angle offset
	ljmp digitWait
rotation_addition:
	cjne a, #03h, rotation_subtraction
	clr C
	mov a, R6
	add a, #01h ; add one to the values
	mov R6, a
	mov a, R7
	add a, #00h ; check if overflow
	mov R7, a
	ljmp digitWait
rotation_subtraction:
	cjne a, #04h, reset
	clr C
	mov a, R6
	subb a, #01h ; subtract one from the values
	mov R6, a
	mov a, R7
	subb a, #00h ; check if overflow
	mov R7, a
	ljmp digitWait
reset:
	clr ea
