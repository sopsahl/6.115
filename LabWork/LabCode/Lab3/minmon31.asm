...
;   *  Modified for Read, Write, Vector			    *
;   *  March 17, 2024    Simon Opsahl			    *
;   *                                               *
;   *************************************************
...
jumtab:
   ...
   .dw readadr            ; command 'r' 12 used
   .dw badcmd             ; command 's' 13
   .dw badcmd             ; command 't' 14
   .dw badcmd             ; command 'u' 15
   .dw vctr               ; command 'v' 16 used
   .dw wrtadr             ; command 'w' 17 used
...
;===============================================================
; command readadr 'v'
; this routine writes to the 4 hex digit address and its 
; successors until an escape or 256 bytes are written
;===============================================================
vctr:
	lcall getbyt           ; get address high byte
	setb acc.7			   ; forces the write to go to RAM           
	lcall prthex
	mov DPH, a 		; moves the address high byte to DPH
	mov DPL, #00h   ; starts DPL at #00h
	vctrLoop:
		vctr_nextLine:
			lcall crlf 
			mov a, DPH ; prints the high
			lcall prthex
			mov a, DPL ; prints the low
			lcall prthex
			mov a, #'=' ; equals sign
			lcall sndchr
		lcall getbyt		    ; gets the value to write in acc
		lcall prthex
		movx @dptr, a			; write the value in acc to mem
		inc dptr  				; increments the data pointer
		mov a, DPL 	
		jnz vctrLoop			; checks if the dpl has returned to zero
	ljmp endloop 
	
;===============================================================
; command readadr 'w'
; this routine writes to the 4 hex digit address with the 2 digit
; hex number that follows 'address'=
;===============================================================
wrtadr:
	lcall getbyt           ; get address high byte
	setb acc.7			   ; forces the write to go to RAM
	...
	pop DPL ; save in DPL, the DPTR low byte
	pop DPH ; save in in DPH, the DPTR high byte
...
;===============================================================
; command readadr 'r'
; this routine reads from the 4 hex digit address which follows
;===============================================================
readadr:
	lcall getbyt           ; get address high byte
	mov dph, a             ; save in in DPH, the DPTR high byte
	lcall prthex
	lcall getbyt          ; get address low byte
	mov dpl, acc		    ; save in DPL, the DPTR low byte
	lcall prthex
    lcall crlf
	movx a, @dptr			; move the data point into the acc
	lcall prthex			; display the ascii of the hex
	ljmp endloop
;===============================================================
; command goaddr  'g'
...
; getbytx verifies that the output is valid
;===============================================================
getbyt:
